...
def _kubecpu(i,uc):
    return f'microk8s.kubectl exec -it {i} -n {G5Conf[f"{uc}-netapp"]} -- /bin/cat /sys/fs/cgroup/cpu/cpuacct.usage'
def _kubemem(i,uc):
    return f'microk8s.kubectl exec -it {i} -n {G5Conf[f"{uc}-netapp"]} -- /bin/cat /sys/fs/cgroup/memory/memory.usage_in_bytes'
...
def cpu_percent(uc):
    ...
    try:
        ...
        #print(last_cpu_usage,last_system_usage)
